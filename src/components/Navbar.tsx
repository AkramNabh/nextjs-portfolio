    ...
    <nav className="sticky top-0 max-w-7xl mx-auto p-5 flex items-center justify-between z-20">
        ...
        <div className="flex items-center space-x-2 ">
          {props.map((s: Social, index: number) => (
            ...
            <SocialIcon
              key={index}
              bgColor={`${
                theme === "dark" ? DarkTheme.socialsBg : LightTheme.socialsBg
              }`}
      ...
      <Motion delay={1} direction={"right"}>
        <div className="md:flex items-center space-x-8 hidden">
          {pathname !== "/projects" && (
            <Link
              className={`text-[#1E1B18] dark:text-[#FFF9FB] font-bold hover:scale-150 duration-200 uppercase tracking-wide md:tracking-widest text-sm hover:text-[#FF7F11] dark:hover:text-[#D65A31] md:text-base`}
              href="/projects"
            >
              Projects
            </Link>
      ...
      <div className="md:hidden">
        ...
        <div
          id="burger"
          className={`sticky top-0 z-25 ${isOpen ? "flex" : "hidden"}`}
        >
